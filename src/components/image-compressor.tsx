...
export default function ImageCompressor() {
  ...
  const compressImages = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      setFiles((prev) => prev.map((f) => (f.id === file.id ? { ...f, status: "compressing" as const } : f)))
      ...
      const compressionRatio = Math.random() * 0.4 + 0.3 // Random between 30% and 70%
      const compressedSize = Math.floor(file.originalSize * compressionRatio)
        ...
        prev.map((f) =>
          f.id === file.id
            ? {
                ...
                progress: 100,
                compressedSize,
                compressedUrl: f.preview, // In a real app, this would be the compressed image URL
              }
            : f,
        ),
      )
  ...
  const downloadAll = () => {
    const completedFiles = files.filter((file) => file.status === "done" && file.compressedUrl)
  ...
  const allCompleted = files.length > 0 && files.every((file) => file.status === "done")
      ...
      <TabsContent value="upload" className="space-y-8">
        <div
          ...
          className={cn(
            ...
            isDragging && "dragging",
            files.length > 0 && "border-dashed border-muted-foreground/20",
          )}
        >
          <input {...getInputProps()} />
          <div className="flex flex-col items-center justify-center gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">Supports JPG, PNG, SVG, GIF, WebP and more</p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <div className="flex gap-3">
                <Button variant="outline" size="sm" onClick={clearAllFiles} disabled={isCompressing}>
                ...
                {allCompleted ? (
                  ...
                  <Button size="sm" onClick={compressImages} disabled={isCompressing || files.length === 0}>
                ...
                <Card key={file.id} className="overflow-hidden">
                  ...
                  <CardContent className="p-4">
                    <div className="space-y-2">
                      ...
                      {file.status === "compressing" && <Progress value={file.progress} className="h-2" />}

                      {file.status === "done" && file.compressedUrl && (
                        <Button variant="outline" size="sm" className="w-full" onClick={() => downloadFile(file)}>
                          <Download className="h-4 w-4 mr-2" />
                          Download
                        </Button>
                      )}
                      ...
                      {file.status === "error" && (
                        <p className="text-xs text-destructive">{file.error || "Error compressing image"}</p>
                      )}
                    </div>
                  </CardContent>
                </Card>
      ...
      <TabsContent value="settings" className="space-y-8">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Adjust these settings to control the compression quality and output format.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-2">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p className="max-w-xs">
                        Higher quality means larger file size. Lower quality means smaller file size but may introduce
                        artifacts.
                      </p>
                    </TooltipContent>
                  </Tooltip>
              ...
              />
              <p className="text-xs text-muted-foreground">Recommended: 70-85% for web images, 85-95% for printing</p>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent>
                      <p className="max-w-xs">
                        Different formats have different compression characteristics. WebP usually offers the best
                        compression.
                      </p>
                    </TooltipContent>
                  </Tooltip>
              ...
              <p className="text-xs text-muted-foreground">
                WebP offers the best compression but may not be supported by all applications
              </p>
            </div>
            ...
            <div className="flex items-center justify-between space-x-2">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">Keep metadata like camera settings and GPS location</p>
              </div>
              <Switch id="preserve-exif" checked={preserveExif} onCheckedChange={setPreserveExif} />
            </div>
          </CardContent>
          <CardFooter>
            <p className="text-xs text-muted-foreground">Settings are applied to all images in the current session</p>
          </CardFooter>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              These settings are for users who need more control over the compression process.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between space-x-2">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">Automatically resize large images</p>
              </div>
            ...
            <div className="flex items-center justify-between space-x-2">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">Create progressive JPGs for faster perceived loading</p>
              </div>
              <Switch defaultChecked={true} />
            </div>
            ...
            <div className="flex items-center justify-between space-x-2">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-muted-foreground">Remove color profile data to reduce file size</p>
              </div>
